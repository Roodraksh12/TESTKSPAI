...
export function ChatPanel() {
  ...
  const send = (text: string) => {
    ...
    setThinking(true);
    setTimeout(() => {
      setMessages((m) => [...m, craftReply(text)]);
      setThinking(false);
    }, 1100);
  };
    ...
    <div className="glass flex h-[calc(100vh-13rem)] min-h-[560px] flex-col overflow-hidden rounded-3xl">
      ...
      <div className="border-t border-hairline p-4">
        <form
          ...
          className="flex items-center gap-2 rounded-full border border-hairline bg-surface-2 px-3 py-2"
        >
          <button type="button" className="rounded-full p-2 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Attach">
          ...
          />
          <button type="button" className="rounded-full p-2 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Voice">
            <Mic className="h-4 w-4" />
          </button>
          <GlassButton variant="primary" size="sm" type="submit" className="!rounded-full !px-3 !py-2">
