...
export async function POST(req: Request) {
  try {
    ...
    const { message, stationId, history = [], pageContext } = await req.json()
...
Keep responses concise and operational.${contextAddition}`
    ...
    const availableTools: OpenAI.Chat.Completions.ChatCompletionTool[] = [
      ...
      },
      {
        ...
        function: {
          ...
          description: "Search for specific IPC or BNS legal sections.",
          parameters: {
            type: "object",
            properties: {
              sectionQuery: { type: "string", description: "The section number, e.g. '379' or '302'" }
            },
      ...
      },
      {
        ...
        function: {
          ...
          description: "Extract named entities (suspects, vehicles, locations, time) from a given text or FIR draft.",
          parameters: {
            type: "object",
            properties: {
              text: { type: "string" }
    ...
    // Filter out highly sensitive intelligence tools if the user is just a Constable
    const filteredTools = availableTools.filter(tool => {
      if (session.user.role === 'Constable') {
        const restrictedTools = ['get_hotspot_summary', 'get_person_connections', 'get_similar_cases'];
        if (restrictedTools.includes(tool.function.name)) return false;
      }
      return true;
    });
    ...
    if (responseMessage.tool_calls) {
      ...
      for (const toolCall of responseMessage.tool_calls) {
        ...
        try {
          ...
          } else if (toolCall.function.name === "fetch_ipc_section") {
            toolResult = await tools.fetchIpcSection(args as any, effectiveStationId)
          } else if (toolCall.function.name === "extract_entities") {
            toolResult = await tools.extractEntities(args as any, effectiveStationId)
          }
        } catch (e) {
