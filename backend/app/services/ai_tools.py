...
from app.services.case_access import create_audit_log, get_case_with_relations, station_filter_sql
from app.services.db import fetch_all, fetch_one


async def search_cases(args: dict[str, Any], station_id: str) -> list[dict[str, Any]]:
    clauses = ['"stationId" = %(stationId)s']
    params: dict[str, Any] = {"stationId": station_id}
...
async def get_person_connections(args: dict[str, Any], _station_id: str) -> list[dict[str, Any]]:
    return fetch_all(
        '''
        SELECT c.*,
...
async def get_hotspot_summary(args: dict[str, Any], station_id: str) -> list[dict[str, Any]]:
...
TOOL_HANDLERS = {
    "search_cases": lambda args, station_id, officer_id, is_sp: search_cases(args, station_id),
    ...
    "get_person_connections": lambda args, station_id, officer_id, is_sp: get_person_connections(args, station_id),
    "get_similar_cases": lambda args, station_id, officer_id, is_sp: get_similar_cases(args, station_id),
    "get_hotspot_summary": lambda args, station_id, officer_id, is_sp: get_hotspot_summary(args, station_id),
