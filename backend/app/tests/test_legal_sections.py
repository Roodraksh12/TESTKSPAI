...
def _ids(result: dict) -> list[str]:
    return [p["bnsSection"] for p in result["predictions"]]


def test_theft_predicts_bns_303_as_primary() -> None:
...
def test_weapon_wording_surfaces_conditional_bns_311() -> None:
    ...
    assert "BNS 311" in _ids(with_weapon)
    weapon_row = next(p for p in with_weapon["predictions"] if p["bnsSection"] == "BNS 311")
    ...
    if "BNS 311" in _ids(without):
        plain_row = next(p for p in without["predictions"] if p["bnsSection"] == "BNS 311")
...
def test_night_entry_triggers_bns_331_4() -> None:
    result = predict_sections("Burglary", "Entry made around 2 am through the balcony.")
    assert "BNS 331(4)" in _ids(result)
...
def test_summary_only_match_is_flagged_as_inferred() -> None:
    ...
    cheating = next(p for p in result["predictions"] if p["bnsSection"] == "BNS 318(4)")
