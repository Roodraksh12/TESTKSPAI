...
export default function HotspotMap() {
  ...
  if (!mapsKey) {
    return (
      <div className="flex flex-1 w-full min-h-[16rem] items-center justify-center rounded-2xl border border-dashed border-hairline bg-surface-2 px-6 text-center">
        <div>
          <p className="text-sm font-medium text-foreground">Google Maps key missing</p>
          <p className="mt-1 text-xs text-muted-foreground">
            Set <code className="text-mono">VITE_GOOGLE_MAPS_API_KEY</code> in{" "}
            <code className="text-mono">frontend/.env</code>, enable Maps JavaScript API in Google Cloud,
            then restart <code className="text-mono">npm run dev</code>.
          </p>
        </div>
      </div>
    );
