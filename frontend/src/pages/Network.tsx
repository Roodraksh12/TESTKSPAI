...
export default function NetworkPage() {
  ...
  const visibleNodes = useMemo(() => {
    let nodes = NETWORK.nodes.filter(
      (n) => activeKinds[n.kind] && (query === "" || n.label.toLowerCase().includes(query.toLowerCase()))
    );
    
    // Isolate mode: if a node is actively clicked, hide everything not directly connected
    if (selectedId) {
