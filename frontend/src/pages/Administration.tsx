...
type GeoOption = { id: string; name: string; districtName?: string; commandRangeName?: string; districtId?: string; rangeId?: string };
...
export default function AdministrationPage() {
  ...
  const isLeaf = Boolean(caps.isLeaf);
  const canInvite = Boolean(caps.canInvite);
  const canFulfill = Boolean(caps.canFulfillResets);
  const isIt = Boolean(caps.isPoliceIt);
  // Police IT uses dedicated /invite and /password-resets pages
  const showInviteHere = canInvite && !isIt;
  const showResetsHere = canFulfill && !isIt;
  ...
  const load = async () => {
    ...
    setError("");
    try {
      const [subtree, stationPayload, crPayload, distPayload, subPayload] = await Promise.all([
        apiRequest("/api/admin/subtree"),
      ...
      setOfficers(subtree.officers || []);
      setStations(stationPayload.stations || []);
      setCommandRanges(crPayload.commandRanges || []);
      setDistricts(distPayload.districts || []);
      setSubdivisions(subPayload.subdivisions || []);
      if (!stationId && stationPayload.stations?.length) {
        setStationId(user?.stationId || stationPayload.stations[0].id);
      }
      if (canFulfill && !isIt) {
      ...
      if (isIt) {
        const s = await apiRequest("/api/admin/smtp-status");
        setSmtp(s);
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (inviteable.length && !inviteable.includes(role)) {
      setRole(inviteable[0]);
    }
  }, [inviteable, role]);
  ...
  const title = useMemo(() => t("admin.title"), [t]);
  ...
  if (isLeaf) {
    return (
      <div className="space-y-4 max-w-xl">
        ...
        <p className="text-sm text-muted-foreground">{t("admin.profileOnly")}</p>
        <div className="rounded-2xl border border-hairline bg-surface p-5 space-y-2">
          <p className="text-sm font-medium">{user?.name}</p>
          <p className="text-xs text-muted-foreground">{user?.badgeId} · {user?.role}</p>
          <p className="text-xs text-muted-foreground">{user?.stationName} · {user?.districtName}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col gap-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">{isIt ? t("nav.officers") : title}</h1>
        <p className="text-sm text-muted-foreground">
          {isIt ? t("admin.subtree") : t("admin.subtitle")}
        </p>
      </div>
      ...
      {showResetsHere && (
        <section className="rounded-2xl border border-hairline bg-surface p-5 space-y-3">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">{t("admin.resetQueue")}</h2>
          {resets.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("admin.noResets")}</p>
          ) : (
            <ul className="space-y-2">
              {resets.map((r) => (
                <li key={r.id} className="flex items-center justify-between gap-3 rounded-xl border border-hairline px-3 py-2">
                  <div>
                    <p className="text-sm font-medium">{r.name} · {r.badgeId}</p>
                    <p className="text-[11px] text-muted-foreground">{r.role}</p>
                  </div>
                  <Button type="button" onClick={() => fulfill(r.id)}>{t("admin.fulfill")}</Button>
                </li>
              ))}
            </ul>
          )}
      ...
      <section className="rounded-2xl border border-hairline bg-surface p-5 space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">{t("admin.subtree")}</h2>
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[11px] uppercase tracking-wider text-muted-foreground border-b border-hairline">
              ...
              <tbody>
                {officers.map((o) => (
                  <tr key={o.id} className="border-b border-hairline/60">
                    ...
                    <td className="py-2 pr-3 font-mono text-xs">{o.badgeId}</td>
                    <td className="py-2 pr-3">{o.role}</td>
                    <td className="py-2 pr-3">{o.stationName || "—"}</td>
                    <td className="py-2">{o.status || "ACTIVE"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {isIt && smtp && (
        <section className="rounded-2xl border border-hairline bg-surface p-5 space-y-2">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">{t("admin.smtp")}</h2>
          <p className="text-sm">
            {smtp.configured ? t("admin.smtpConfigured") : t("admin.smtpMissing")}
          </p>
          <p className="text-xs text-muted-foreground">
            Host: {smtp.host || "—"} · Port: {smtp.port} · From: {smtp.from || "—"}
          </p>
        </section>
      )}
    </div>
