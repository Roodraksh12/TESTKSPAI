...
import { useFirQueue } from "@/lib/fir-queue";
import { FirQueuePanel } from "@/components/scrb/fir-queue-panel";
...
export default function FIRUploadPage() {
  ...
  // When the job being reviewed finishes, load its result into the form.
  const activeJob = jobs.find((j) => j.jobId === activeJobId) || null;
  useEffect(() => {
    if (activeJob?.status === "done" && activeJob.result && phase !== "done") {
    ...
    if (activeJob?.status === "error") {
      setError(activeJob.error || "Processing failed");
    }
  }, [activeJob, phase]);
  ...
  const handleMagicDraft = async () => {
    ...
    try {
      const data = await apiRequest("/api/ai/draft-fir", {
        method: "POST",
  ...
  // Dispatches to the server queue and returns immediately. The officer can
  // navigate away, or queue another scan, while this one processes.
    ...
    <div className="space-y-6 max-w-[1400px] mx-auto p-4 sm:p-6 lg:p-8">
      ...
      {phase === "done" && extractedData && (
        <Card accent="teal" className="p-6 sm:p-8">
          ...
          {/* Live section preview from the editable fields above — updates as the
              officer corrects the crime type or narrative, before anything is saved. */}
          <LegalSectionsPanel
            className="mt-5"
            crimeType={extractedData.crimeType}
            summary={[extractedData.narrativeSummary, extractedData.modusOperandi].filter(Boolean).join(" ")}
          />
